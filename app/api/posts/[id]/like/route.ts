...
async function mutate(request: Request, context: Context, enabled: boolean) {
  try { assertSameOrigin(request); const auth = await requireAuth(request); await enforceRateLimit(auth.id, "social"); const { id } = await context.params; await setPostRelation("likes", id, auth.id, enabled); return ok({ liked: enabled }); } catch (error) { return routeError(error, request); }
}
