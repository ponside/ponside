...
async function mutate(request: Request, context: Context, enabled: boolean) {
  try { assertSameOrigin(request); const auth = await requireAuth(request); await enforceRateLimit(auth.id, "social"); const { id } = await context.params; await setFollow(id, auth.id, enabled); return ok({ following: enabled }); } catch (error) { return routeError(error, request); }
}
